...
class SZ3Compressor(BaseCompressor):
    """
    SZ3Compressor is a class that compresses and decompresses model parameters using SZ3 lossy compressor.
    :param compressor_config: the configuration of the compressor
        - lossless_compressor: the lossless compressor used in combination with SZ3 (blosc, gzip, lzma, zstd, zlib)
        - error_bounding_mode: the error bounding mode used in SZ3 (ABS, REL, ABS_AND_REL, ABS_OR_REL, PSNR, NORM, PW_REL)
        - error_bound (float): the error bound used in SZ3
    ...
    def __init__(self, compressor_config: DictConfig):
        ...
        self.compressor_lib_path = (
            os.path.join(
                pathlib.Path.home(), ".appfl/.compressor/SZ3/build/tools/sz3c/libSZ3c"
            )
            + ext
        )
