...
class ServerAgent:
    ...
    def global_update(
        ...
        **kwargs,
        ...
        meta_data = kwargs
        ckks_context = ts.context_from(meta_data["ckks_context"])
        self.logger.info(f"ckks_context={ckks_context}")
        for name, serialized_param in local_model.items():
            local_model[name] = ts.ckks_vector_from(ckks_context, serialized_param)
